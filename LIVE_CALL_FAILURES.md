...
| LCF-006 | reopened | `call_22ee89f4f6d258b103a3df580aa`; regression evidence `call_d14b27acbd755a7ef3edcff30f0` | Completed intake did not reliably end the Retell call; latest observed failure did not properly terminate after the caller reached the close path. | Once required details are confirmed and the caller indicates the conversation is complete, send `end_call: true`, stop producing additional intake prompts, and avoid leaving the Retell call open. | Existing tests: `completed_intake_ends_retell_call` and `final_close_check_yes_ends_call` in `tests/main_tests.cpp`. Needs added replay/fixture coverage for the exact latest transcript ending path. |
...
| LCF-W004 | watching | Latest ending regression from `call_d14b27acbd755a7ef3edcff30f0` | The deterministic close-check tests may pass while a real Retell transcript still fails to terminate because the live transcript wording, event trigger, or final-response path differs from the unit-test path. | Add a replayable transcript test for the exact live ending sequence and verify the WebSocket response includes `end_call: true` on the final turn. |
