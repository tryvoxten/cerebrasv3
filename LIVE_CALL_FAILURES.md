...
| LCF-006 | deployed | `call_22ee89f4f6d258b103a3df580aa` | Completed intake did not reliably end the Retell call. | Once required details are confirmed, send `end_call: true` and avoid restarting questioning. | `completed_intake_ends_retell_call` and `phone_confirmation_closes_without_final_reconfirmation` in `tests/main_tests.cpp`. |
